'use client'
// src/app/progresso/page.tsx — Evolução + Stats numa página só
...
export default function ProgressoPage() {
  ...
  const [stats,   setStats]   = useState({ consistency:0, xpWeek:0, sleep:0, energy:0, prevConsistency:0, xpTrend:0, thisDone:0, thisTotal:0, checkinsToday:0 })
  ...
  useEffect(() => {
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      const [prof, ub, weekly,
        ...
        supabase.from('habit_logs').select('date,completed,habit_id').eq('user_id',user.id).gte('date',since14).lt('date',since7),
        supabase.from('habit_logs').select('date,completed').eq('user_id',user.id).gte('date',since28),
        supabase.from('checkins').select('date,phase,xp_earned').eq('user_id',user.id).gte('date',since14).lt('date',since7),
        supabase.from('habits').select('id,area,xp_reward').eq('user_id',user.id).eq('active',true),
      ])

      setProfile(prof); setBadges(ub as UserBadge[])
      ...
      const areaList: AreaProgress[] = Object.entries(AREA_META).map(([key, meta]) => {
        ...
        const done  = (weekly.logs as LogRow[]).filter((l: LogRow) => ids.includes(l.habit_id) && l.completed).length
        return { key, label: meta.label, icon: meta.icon, color: meta.color, pct: total > 0 ? Math.round(done/total*100) : 0, done, total }
      })
      setAreas(areaList)
      ...
      const xpByDay = Array.from({length:7},(_,i) => {
        ...
        const habXP  = (weekly.logs as LogRow7[]).filter(l => l.date===day && l.completed).reduce((a:number,l:LogRow7) => a+(xpMap[l.habit_id]??10),0)
        const ciXP   = (weekly.checkins as CIRow[]).filter(c => c.date===day).reduce((a:number,c:CIRow) => a+(c.xp_earned??0),0)
      ...
      const hm = Array.from({length:28},(_,i) => {
        ...
        const day = format(d,'yyyy-MM-dd')
        return Math.min(3,(logs28 as Log28[] ?? []).filter(l => l.date===day && l.completed).length)
      })
      setHeatmap(hm)
      ...
      const prevXP  = (logs14 as LogH[] ?? []).filter(l => l.completed).reduce((a:number,l:LogH)=>a+(xpMap[l.habit_id]??10),0) + (ci14 as CIXp[] ?? []).reduce((a:number,c:CIXp)=>a+(c.xp_earned??0),0)
      const ciAll   = (weekly.checkins as CIRow[]).filter(c => c.energy)
      const slAll   = (weekly.checkins as CIRow[]).filter(c => c.sleep_hours)
      const avgE    = ciAll.length>0 ? Math.round(ciAll.reduce((a:number,c:CIRow)=>a+(c.energy??0),0)/ciAll.length*10)/10 : 0
      const avgS    = slAll.length>0 ? Math.round(slAll.reduce((a:number,c:CIRow)=>a+(c.sleep_hours??0),0)/slAll.length*10)/10 : 0
      const checkinsToday = (weekly.checkins as CIRow[]).filter(c=>c.date===today).length
      setStats({ consistency, prevConsistency, xpWeek, xpTrend:xpWeek-prevXP, sleep:avgS, energy:avgE, thisDone, thisTotal, checkinsToday })
      setLoading(false)
  ...
  return (
    <main style={{paddingBottom:100,minHeight:'100vh'}}>
      ...
      <div style={{padding:'28px 20px 0'}}>
        <h1 style={{fontFamily:'Syne, sans-serif',fontWeight:700,fontSize:22,marginBottom:16}}>Progresso</h1>
        <div style={{display:'flex',background:'var(--bg2)',borderRadius:14,padding:4,gap:4,border:'0.5px solid var(--border)'}}>
          {[{k:'evolucao',l:'Evolução',i:'⚡'},{k:'stats',l:'Dashboard',i:'📊'}].map(({k,l,i}) => (
            <button key={k} onClick={()=>setTab(k as AppTab)} style={{
      ...
      {tab==='evolucao' && (
        <>
          {/* Level card */}
          <div style={{margin:'16px 20px 0',padding:'22px 20px',borderRadius:20,background:'var(--bg2)',border:'0.5px solid rgba(232,168,56,.25)',position:'relative',overflow:'hidden'}}>
            ...
            <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',marginBottom:16}}>
              <div>
                <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:6}}>
                  <div style={{background:'rgba(232,168,56,.12)',border:'0.5px solid rgba(232,168,56,.3)',borderRadius:10,padding:'4px 12px',fontFamily:'Syne, sans-serif',fontWeight:800,fontSize:13,color:'var(--gold)',letterSpacing:'.5px'}}>NÍV. {level}</div>
                  <div style={{fontFamily:'Syne, sans-serif',fontWeight:700,fontSize:18,color:'var(--text1)'}}>{profile.title}</div>
                </div>
                <div style={{fontSize:12,color:'var(--text3)',lineHeight:1.4}}>{TITLES[profile.title]??''}</div>
              </div>
              <div style={{textAlign:'right'}}>
                <div style={{fontFamily:'Syne, sans-serif',fontWeight:800,fontSize:30,color:'var(--text1)',lineHeight:1}}>{xp.toLocaleString('pt-PT')}</div>
            ...
            <div style={{background:'var(--bg3)',borderRadius:100,height:8,marginBottom:6}}>
              <div style={{height:'100%',borderRadius:100,background:'linear-gradient(90deg,var(--gold),#F0C060)',width:`${pct}%`,transition:'width .8s ease'}}/>
            </div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:8,padding:'12px 20px 0'}}>
            {[
              ...
              {l:'Recorde',   v:`${profile.streak_best}`,   u:'dias',c:'var(--accent)', i:'🏅'},
              {l:'Conquistas',v:`${earned.size}/${ALL_BADGES.length}`,u:'',c:'var(--teal)',i:'✨'},
            ].map(({l,v,u,c,i,flame})=>(
              <div key={l} style={{background:'var(--bg2)',border:'0.5px solid var(--border)',borderRadius:14,padding:'14px 12px',textAlign:'center'}}>
                <div style={{fontSize:20,marginBottom:6,display:'inline-block',...(flame?{animation:'flame 1.8s ease-in-out infinite',transformOrigin:'bottom center'}:{})}}>{i}</div>
                <div style={{fontFamily:'Syne, sans-serif',fontWeight:700,fontSize:20,color:c,lineHeight:1}}>{v}</div>
          ...
          <div style={{padding:'20px 20px 0'}}>
            <div style={{fontFamily:'Syne, sans-serif',fontSize:12,fontWeight:600,color:'var(--text3)',textTransform:'uppercase',letterSpacing:1,marginBottom:10}}>Equilíbrio de vida</div>
          ...
          <div style={{padding:'16px 20px 0'}}>
            <div style={{fontFamily:'Syne, sans-serif',fontSize:12,fontWeight:600,color:'var(--text3)',textTransform:'uppercase',letterSpacing:1,marginBottom:10}}>Progresso por Área — 30 dias</div>
                ...
                <div key={a.key} style={{background:'var(--bg2)',border:'0.5px solid var(--border)',borderRadius:14,padding:'13px 16px'}}>
                  <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:8}}>
                    ...
                    <div style={{display:'flex',alignItems:'center',gap:8}}>
                      ...
                      <span style={{fontFamily:'Syne, sans-serif',fontWeight:700,fontSize:14,color:a.pct>=70?'var(--teal)':a.pct>=40?'var(--gold)':'var(--text3)',minWidth:36,textAlign:'right'}}>{a.total>0?`${a.pct}%`:'—'}</span>
                    </div>
                  </div>
                  <div style={{background:'var(--bg3)',borderRadius:100,height:5}}>
                    <div style={{height:'100%',borderRadius:100,width:`${a.pct}%`,background:a.pct>=70?'var(--teal)':a.pct>=40?'var(--gold)':a.color,transition:'width .8s ease'}}/>
                  </div>
                </div>
              ))}
            </div>
          ...
          {/* Conquistas */}
          <div style={{padding:'20px 20px 0'}}>
            <div style={{fontFamily:'Syne, sans-serif',fontSize:12,fontWeight:600,color:'var(--text3)',textTransform:'uppercase',letterSpacing:1,marginBottom:10}}>Conquistas</div>
              ...
              {ALL_BADGES.map(b => {
                ...
                return (
                  <button key={b.key} onClick={()=>setActiveBadge(isA?null:b.key)} style={{
                    ...
                    <span style={{fontSize:10,color:isE?'var(--text1)':'var(--text3)',textAlign:'center',lineHeight:1.3}}>{b.name}</span>
                    {isE&&<span style={{fontSize:9,color:'var(--teal)'}}>+{b.xp} XP</span>}
                    {isA&&!isE&&sp&&(
                      <div style={{width:'100%',marginTop:2}}>
                        <div style={{background:'var(--bg3)',borderRadius:100,height:3}}>
                          <div style={{height:'100%',borderRadius:100,background:'var(--gold)',width:`${Math.round(sp.cur/sp.max*100)}%`}}/>
                        </div>
                        <div style={{fontSize:9,color:'var(--gold)',textAlign:'center',marginTop:3}}>{sp.cur}/{sp.max}</div>
                      </div>
                    )}
          ...
          <div style={{margin:'20px 20px 0',padding:'16px',borderRadius:14,background:'var(--bg2)',border:'0.5px solid var(--border)'}}>
            ...
            {[{t:'Recruta',min:1,max:2},{t:'Consistente',min:3,max:4},{t:'Focado',min:5,max:7},{t:'Estrategista',min:8,max:10},{t:'Imparável',min:11,max:14},{t:'Antifrágil',min:15,max:20}].map(t=>{
              ...
              return (
                <div key={t.t} style={{display:'flex',alignItems:'center',gap:10,padding:'5px 0'}}>
                  <div style={{width:8,height:8,borderRadius:'50%',flexShrink:0,background:isPast?'var(--teal)':isCur?'var(--gold)':'var(--bg3)'}}/>
                  <div style={{flex:1,fontSize:13,color:isCur?'var(--text1)':'var(--text3)',fontWeight:isCur?600:400}}>{t.t}</div>
      ...
      {tab==='stats' && (
        <>
          ...
          <div style={{display:'flex',alignItems:'flex-end',justifyContent:'space-between',padding:'16px 20px 0'}}>
            ...
            <div style={{textAlign:'right'}}>
              <div style={{fontFamily:'Syne, sans-serif',fontWeight:700,fontSize:18,color:'var(--gold)'}}>{stats.checkinsToday}/3</div>
          ...
          <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,padding:'10px 20px 0'}}>
            {[
              {l:'Consistência',v:`${stats.consistency}%`,trend:`${stats.consistency>=stats.prevConsistency?'↑':'↓'} vs semana passada`,c:'var(--teal)',up:stats.consistency>=stats.prevConsistency},
              {l:'XP esta semana',v:`${stats.xpWeek}`,trend:`${stats.xpTrend>=0?'↑ +':'↓ '}${Math.abs(stats.xpTrend)} vs ant.`,c:'var(--gold)',up:stats.xpTrend>=0},
              {l:'Energia média',v:stats.energy>0?`${stats.energy}/10`:'—',sub:stats.energy>=7?'Boa semana ⚡':stats.energy>0?'Recupera o sono':'Sem dados',c:'var(--accent)'},
              {l:'Sono médio',v:stats.sleep>0?`${stats.sleep}h`:'—',sub:stats.sleep>=7?'Dentro do ideal':stats.sleep>0?'Abaixo do ideal':'Sem dados',c:'var(--text1)'},
            ].map(({l,v,trend,sub,c,up})=>(
              <div key={l} style={{background:'var(--bg2)',border:'0.5px solid var(--border)',borderRadius:14,padding:14}}>
                ...
                <div style={{fontFamily:'Syne, sans-serif',fontWeight:700,fontSize:22,color:c,marginBottom:3}}>{v}</div>
          ...
          {/* Mentor */}
          <div style={{margin:'10px 20px 0',padding:'14px 16px',borderRadius:14,background:'var(--bg2)',border:'0.5px solid rgba(30,203,180,.18)',fontSize:13,color:'var(--text2)',lineHeight:1.6}}>
            ...
            {' '}<span style={{color:'var(--gold)'}}>
              Próxima conquista: {profile.streak_current<7?`streak de 7 dias — faltam ${7-profile.streak_current}`:profile.streak_current<14?`streak de 14 dias — faltam ${14-profile.streak_current}`:`streak de 30 dias — faltam ${30-profile.streak_current}`} dias.
            </span>
          </div>
          ...
          <div style={{padding:'12px 20px 0'}}>
                ...
                {ready&&(
                  <ResponsiveContainer width="100%" height={150}>
                    <BarChart data={xpData} barGap={2} barCategoryGap="30%">
                      ...
                      <YAxis hide/>
                      <Tooltip contentStyle={TT} formatter={(v:number,n:string)=>[v,n==='hab'?'Hábitos XP':'Check-in XP']}/>
            ...
            <div style={{background:'var(--bg2)',border:'0.5px solid var(--border)',borderRadius:16,padding:16,marginBottom:12}}>
              <div style={{fontSize:12,color:'var(--text3)',marginBottom:10}}>Constância — 4 semanas</div>
              <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:4,marginBottom:4}}>
                {['S','T','Q','Q','S','S','D'].map((d,i)=><div key={i} style={{fontSize:9,color:'var(--text3)',textAlign:'center'}}>{d}</div>)}
              </div>
              <div style={{display:'grid',gridTemplateColumns:'repeat(7,1fr)',gap:4}}>
                {heatmap.map((v,i)=><div key={i} style={{background:hmColors[v],borderRadius:4,aspectRatio:'1'}}/>)}
              </div>
            ...
            <div style={{background:'var(--bg2)',border:'0.5px solid var(--border)',borderRadius:14,padding:'14px 16px',marginBottom:12}}>
              ...
              {[
                ...
                <div key={l} style={{display:'flex',justifyContent:'space-between',fontSize:13,marginBottom:6}}>
                  ...
                  <span style={{color:c,fontFamily:'Syne, sans-serif',fontWeight:600}}>{v}</span>
                </div>
              ))}
            </div>
          </div>
