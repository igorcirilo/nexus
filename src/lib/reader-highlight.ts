 ...
 * dourado quando o registo vem vazio ou com lixo (dados antigos ou de outra
 * origem nunca devem partir a pintura da página).
 */
export function normalizeHighlightColor(color: string | null | undefined): string {
  if (!color) return DEFAULT_HIGHLIGHT_COLOR
  return /^#([0-9a-f]{3}|[0-9a-f]{6})$/i.test(color.trim()) ? color.trim() : DEFAULT_HIGHLIGHT_COLOR
}
